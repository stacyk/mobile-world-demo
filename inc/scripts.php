...
function wpdemo_font_url() {
	...
	*/
	$roboto = _x( 'on', 'Roboto font: on or off', 'wpdemo' );
	$open_sans = _x( 'on', 'Open Sans font: on or off', 'wpdemo' );
...
function wpdemo_scripts() {
	...
	// Register styles.
	wp_register_style( 'wpdemo-google-font', wpdemo_font_url(), array(), null );

	// Enqueue styles.
	wp_enqueue_style( 'wpdemo-google-font' );
	wp_enqueue_style( 'animate.css' );
	wp_enqueue_style( 'wpdemo-style', get_stylesheet_directory_uri() . '/style' . $suffix . '.css', array(), $version );

	// Enqueue scripts.
	wp_enqueue_script( 'wpdemo-project', get_template_directory_uri() . '/assets/js/project' . $suffix . '.js', array( 'jquery' ), $version, true );
...
if ( class_exists( 'WDS_Simple_Page_Builder' ) && version_compare( WDS_Simple_Page_Builder::VERSION, '1.6', '>=' ) ) :
	 ...
	 */
	function wpdemo_enqueue_page_builder_scripts() {
	...
	add_action( 'wds_page_builder_after_load_parts', 'wpdemo_enqueue_page_builder_scripts' );
 ...
 */
function wpdemo_include_svg_definitions() {
